...
/**
 * Class to manage bridging between plugin duties and implementation duties, for example event
 * handling and plugin management.
 */
...
public final class PluginManager {
  ...
  /**
   * Checks if the command is registered and can possibly be executed by the sender (without taking
   ...
   *
   * @param sender the sender executing the command
   * @param commandLine the complete command line including command name and arguments
   * @param tabResults list to place tab results into. If this list is non null then the command
   *     will not be executed and tab results will be returned instead.
  ...
  public boolean dispatchCommand(
    ...
    if (!command.hasPermission(sender)) {
      if (tabResults == null) {
        sender.sendMessage(
            (command.getPermissionMessage() == null)
                ? proxy.getTranslation("no_permission")
    ...
    try {
      ...
      sender.sendMessage(
          ChatColor.RED
              + "An internal error occurred whilst executing this command, please check the console"
              + " log for details.");
    ...
    for (Map.Entry<String, PluginDescription> entry : toLoad.entrySet()) {
      ...
      if (!enablePlugin(pluginStatuses, new Stack<PluginDescription>(), plugin)) {
        ProxyServer.getInstance()
            .getLogger()
            .log(Level.WARNING, "Failed to enable {0}", entry.getKey());
      }
    }
    toLoad.clear();
    toLoad = null;
    ...
    for (Plugin plugin : plugins.values()) {
      try {
        ...
        ProxyServer.getInstance()
            ...
            .log(
                Level.WARNING,
                "Exception encountered when loading plugin: " + plugin.getDescription().getName(),
                t);
      }
    }
  }
    ...
    for (String dependName : dependencies) {
      ...
      if (dependStatus == null) {
        if (dependStack.contains(depend)) {
          ...
          ProxyServer.getInstance()
              .getLogger()
              .log(Level.WARNING, "Circular dependency detected: {0}", dependencyGraph);
          status = false;
        } else {
      ...
      if (dependStatus == Boolean.FALSE
          && plugin.getDepends().contains(dependName)) // only fail if this wasn't a soft dependency
      {
        ProxyServer.getInstance()
            .getLogger()
            .log(
      ...
      try {
        URLClassLoader loader =
            new PluginClassloader(
                ...
                plugin,
                plugin.getFile(),
                (libraryLoader != null) ? libraryLoader.createLoader(plugin) : null);
    ...
    for (File file : folder.listFiles()) {
      if (file.isFile() && file.getName().endsWith(".jar")) {
        try (JarFile jar = new JarFile(file)) {
          ...
          ProxyServer.getInstance()
              .getLogger()
              .log(Level.WARNING, "Could not load plugin from file " + file, ex);
        }
      }
    }
  }
  ...
  /**
   * Dispatch an event to all subscribed listeners and return the event once it has been handled by
   * these listeners.
   *
  ...
  public <T extends Event> T callEvent(T event) {
    ...
    if (elapsed > 250000000) {
      ProxyServer.getInstance()
          ...
          .log(
              Level.WARNING,
              "Event {0} took {1}ms to process!",
              new Object[] {event, elapsed / 1000000});
    }
    return event;
  }

  /**
   * Register a {@link Listener} for receiving called events. Methods in this Object which wish to
   * receive events must be annotated with the {@link EventHandler} annotation.
   *
  ...
  public void registerListener(Plugin plugin, Listener listener) {
    for (Method method : listener.getClass().getDeclaredMethods()) {
      Preconditions.checkArgument(
          ...
          "Listener %s has registered using deprecated subscribe annotation! Please update to"
              + " @EventHandler.",
          listener);
    }
