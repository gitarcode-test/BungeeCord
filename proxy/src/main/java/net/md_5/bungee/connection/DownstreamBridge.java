...
public class DownstreamBridge extends PacketHandler {
  ...
  // #3246: Recent versions of MinecraftForge alter Vanilla behaviour and require a command so that
  // the executable flag is set
  // If the flag is not set, then the command will appear and successfully tab complete, but cannot
  ...
  public void handle(PacketWrapper packet) throws Exception {
    ...
    if (rewrite != null && con.getCh().getEncodeProtocol() == Protocol.GAME) {
      rewrite.rewriteClientbound(
          packet.buf,
          con.getServerEntityId(),
          con.getClientEntityId(),
  ...
  public void handle(KeepAlive alive) throws Exception {
    ...
    if (timeout <= 0
        || server.getKeepAlives().size()
            < timeout
                / 50) // Some people disable timeout, otherwise allow a theoretical maximum of 1
                      // keepalive per tick
    {
      server
          .getKeepAlives()
        ...
        serverScoreboard.addObjective(
            new Objective(
                ...
                (objective.getValue().isLeft())
                    ? objective.getValue().getLeft()
                    : ComponentSerializer.toString(objective.getValue().getRight()),
  ...
  public void handle(ScoreboardScoreReset scoreboardScoreReset) throws Exception {
    ...
    // TODO: Expand score API to handle objective values. Shouldn't matter currently as only used
    // for removing score entries.
  ...
  public void handle(PluginMessage pluginMessage) throws Exception {
    ...
    PluginMessageEvent event =
        new PluginMessageEvent(
            server, con, pluginMessage.getTag(), pluginMessage.getData().clone());
    ...
    if (pluginMessage
      ...
      Preconditions.checkState(
          !serverBrand.contains(bungee.getName()), "Cannot connect proxy to itself!");

      brand = ByteBufAllocator.DEFAULT.heapBuffer();
      DefinedPacket.writeString(
          bungee.getName() + " (" + bungee.getVersion() + ")" + " <- " + serverBrand, brand);
    ...
    if (pluginMessage.getTag().equals("BungeeCord")) {
      ...
      // Check we haven't set out to null, and we have written data, if so reply back back along the
      // BungeeCord channel
      if (out != null) {
  ...
  public void handle(TabCompleteResponse tabCompleteResponse) throws Exception {
    ...
    if (commands == null) {
      commands =
          Lists.transform(
              tabCompleteResponse.getSuggestions().getList(),
      ...
      if (last != null) {
        ...
        commands.addAll(
            bungee.getPluginManager().getCommands().stream()
                .filter(
                    (entry) -> {
                      String lowerCase = entry.getKey().toLowerCase(Locale.ROOT);
                      return lowerCase.startsWith(commandName)
                          && entry.getValue().hasPermission(con)
                          && !bungee.getDisabledCommands().contains(lowerCase);
                    })
    ...
    TabCompleteResponseEvent tabCompleteResponseEvent =
        new TabCompleteResponseEvent(server, con, new ArrayList<>(commands));
    if (!bungee.getPluginManager().callEvent(tabCompleteResponseEvent).isCancelled()) {
      ...
      if (!commands.equals(tabCompleteResponseEvent.getSuggestions())) {
        if (tabCompleteResponse.getCommands() != null) {
          ...
          tabCompleteResponse.setSuggestions(
              new Suggestions(
                  range,
                  Lists.transform(
                      tabCompleteResponseEvent.getSuggestions(),
  ...
  public void handle(Commands commands) throws Exception {
    ...
    for (Map.Entry<String, Command> command : bungee.getPluginManager().getCommands()) {
      if (!bungee.getDisabledCommands().contains(command.getKey())
        ...
        CommandNode dummy =
            LiteralArgumentBuilder.literal(command.getKey())
  ...
  public void handle(ServerData serverData) throws Exception {
    // 1.19.4 doesn't allow empty MOTD and we probably don't want to simulate a ping event to get
    // the "correct" one
