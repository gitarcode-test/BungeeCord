...
public class InitialHandler extends PacketHandler implements PendingConnection {
  ...
  @Getter private PluginMessage brandMessage;
  @Getter private final Set<String> registeredChannels = new HashSet<>();
  ...
  private boolean canSendKickMessage() {
    return thisState == State.USERNAME
    ...
    Callback<ServerPing> pingBack =
        new Callback<ServerPing>() {
          ...
          public void done(ServerPing result, Throwable error) {
            ...
            Callback<ProxyPingEvent> callback =
                new Callback<ProxyPingEvent>() {
                  @Override
                    ...
                    if (v1_5) {
                      kickMessage =
                          ChatColor.DARK_BLUE
                              ...
                              + 127
                              + '\00'
                              + legacy.getVersion().getName()
                              + '\00'
                              + getFirstLine(legacy.getDescription())
                              + '\00'
                              + ((legacy.getPlayers() != null)
                      ...
                      // Clients <= 1.3 don't support colored motds because the color char is used
                      // as delimiter
                      kickMessage =
  ...
  public void handle(Handshake handshake) throws Exception {
    ...
    // add their own data to the end of the string. So, we just take everything from the \0
    // character
  ...
  public void handle(LoginRequest loginRequest) throws Exception {
    ...
    Callback<PreLoginEvent> callback =
        new Callback<PreLoginEvent>() {
  ...
  public void handle(final EncryptionResponse encryptResponse) throws Exception {
    ...
    Preconditions.checkState(
        EncryptionUtil.check(loginRequest.getPublicKey(), encryptResponse, request),
        "Invalid verification");

    SecretKey sharedKey = EncryptionUtil.getSecret(encryptResponse, request);
    BungeeCipher decrypt = EncryptionUtil.getCipher(false, sharedKey);
    ch.addBefore(
        PipelineUtils.FRAME_DECODER, PipelineUtils.DECRYPT_HANDLER, new CipherDecoder(decrypt));
    BungeeCipher encrypt = EncryptionUtil.getCipher(true, sharedKey);
    ch.addBefore(
        PipelineUtils.FRAME_PREPENDER, PipelineUtils.ENCRYPT_HANDLER, new CipherEncoder(encrypt));
          ...
          public void done(String result, Throwable error) {
            if (error == null) {
              ...
              bungee
                  ...
                  .log(
                      Level.SEVERE,
                      "Error authenticating " + getName() + " with minecraft.net",
                      error);
            }
          }
        };
  ...
  private void finish() {
    offlineId =
        UUID.nameUUIDFromBytes(("OfflinePlayer:" + getName()).getBytes(StandardCharsets.UTF_8));
    if (uniqueId == null) {
      uniqueId = offlineId;
    }
    ...
    Callback<LoginEvent> complete =
        new Callback<LoginEvent>() {
          @Override
  ...
  private void finish2() {
    ...
    Callback<PostLoginEvent> complete =
        new Callback<PostLoginEvent>() {
          @Override
    ...
    synchronized (requestedCookies) {
      ...
      if (future != null) {
        if (future.cookie.equals(cookieResponse.getCookie())) {
          Preconditions.checkState(
              future == requestedCookies.poll(), "requestedCookies queue mismatch");
        } else {
  ...
  public void setOnlineMode(boolean onlineMode) {
    Preconditions.checkState(
        thisState == State.USERNAME, "Can only set online mode status whilst state is username");
  ...
  public void setUniqueId(UUID uuid) {
    Preconditions.checkState(
        thisState == State.USERNAME, "Can only set uuid while state is username");
  ...
  public void relayMessage(PluginMessage input) throws Exception {
    ...
    } else if (input.getTag().equals("UNREGISTER")
        || input.getTag().equals("minecraft:unregister")) {
      String content = new String(input.getData(), StandardCharsets.UTF_8);
  ...
  public CompletableFuture<byte[]> retrieveCookie(String cookie) {
    Preconditions.checkState(
        getVersion() >= ProtocolConstants.MINECRAFT_1_20_5,
        "Cookies are only supported in 1.20.5 and above");
    Preconditions.checkState(
        loginRequest != null, "Cannot retrieve cookies for status or legacy connections");

    if (cookie.indexOf(':') == -1) {
      // if we request an invalid resource location (no prefix) the client will respond with
      // "minecraft:" prefix
