...
public class UpstreamBridge extends PacketHandler {
  ...
  public void disconnected(ChannelWrapper channel) throws Exception {
    ...
    if (con.getServer() != null) {
      ...
      newPacket.setUuids(new UUID[] {con.getRewriteId()});
  ...
  public void handle(PacketWrapper packet) throws Exception {
    ...
    if (server != null && server.isConnected()) {
      ...
      // #3527: May still have old packets from client in game state when switching server to
      // configuration state - discard those
  ...
  public void handle(KeepAlive alive) throws Exception {
    ...
    if (keepAliveData != null && alive.getRandomId() == keepAliveData.getId()) {
      Preconditions.checkState(
          keepAliveData == con.getServer().getKeepAlives().poll(), "keepalive queue mismatch");
  ...
  private String handleChat(String message) {
    ...
    if (!bungee.getPluginManager().callEvent(chatEvent).isCancelled()) {
      ...
      if (!chatEvent.isCommand()
          || !bungee.getPluginManager().dispatchCommand(con, message.substring(1))) {
        return message;
      }
    }
    throw CancelSendSignal.INSTANCE;
  ...
  public void handle(TabCompleteRequest tabComplete) throws Exception {
    ...
    if (isCommand) {
      isRegisteredCommand =
          bungee
              .getPluginManager()
              .dispatchCommand(con, tabComplete.getCursor().substring(1), suggestions);
    }

    TabCompleteEvent tabCompleteEvent =
        new TabCompleteEvent(con, con.getServer(), tabComplete.getCursor(), suggestions);
    ...
    if (!results.isEmpty()) {
      // Unclear how to handle 1.13 commands at this point. Because we don't inject into the command
      // packets we are unlikely to get this far unless
      ...
      if (con.getPendingConnection().getVersion() < ProtocolConstants.MINECRAFT_1_13) {
        ...
        con.unsafe()
            .sendPacket(
                new TabCompleteResponse(
                    tabComplete.getTransactionId(), new Suggestions(range, brigadier)));
      }
      throw CancelSendSignal.INSTANCE;
    }
  ...
  public void handle(PluginMessage pluginMessage) throws Exception {
    ...
    if (BungeeCord.getInstance().config.isForgeSupport()) {
      ...
      if (pluginMessage.getTag().equals("FML")
          && pluginMessage.getStream().readUnsignedByte() == 1) {
        throw CancelSendSignal.INSTANCE;
      }
      ...
      if (con.getServer() != null
          && !con.getServer().isForgeServer()
          && pluginMessage.getData().length > Short.MAX_VALUE) {
        // Drop the packet if the server is not a Forge server and the message was > 32kiB (as
        // suggested by @jk-5)
