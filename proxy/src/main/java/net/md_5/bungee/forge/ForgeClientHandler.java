...
public class ForgeClientHandler {
  ...
  @NonNull private final UserConnection con;
  ...
  /** Gets or sets a value indicating whether the '\00FML\00' token was found in the handshake. */
  @Getter @Setter private boolean fmlTokenInHandshake = false;
  ...
  /**
   * Resets the client handshake state to HELLO, and, if we know the handshake has been completed
   * before, send the reset packet.
   */
  ...
  public void setServerIdList(PluginMessage idList) throws IllegalArgumentException {
    if (!idList.getTag().equalsIgnoreCase(ForgeConstants.FML_HANDSHAKE_TAG)
        || idList.getData()[0] != 3) {
  ...
  /**
   * Returns whether we know if the user is a forge user. In FML 1.8, a "FML" token is included in
   * the initial handshake. We can use that to determine if the user is a Forge 1.8 user.
   *
