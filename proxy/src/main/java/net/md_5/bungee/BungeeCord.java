...
public class BungeeCord extends ProxyServer {
  ...
  public BungeeCord() throws IOException {
    // Java uses ! to indicate a resource inside of a jar/zip/other container. Running Bungee from
    // within a directory that has a ! will cause this to muck up.
    Preconditions.checkState(
        new File(".").getAbsolutePath().indexOf('!') == -1,
    ...
    // Furthermore this behaviour only occurs in specific combinations due to the parsing done by
    // jansi.
    // For example test-test works fine, but test-test-test does not! In order to avoid this all
    // together but
    // still keep our versions the same as they were, we set the override property to the
    // essentially garbage version
    ...
    // But we still want to log these records, so we add our own handler which forwards the
    // LogRecord to the BungeeLogger.
    // This way we skip the err stream and the problem of only getting a string without context, and
    // can handle the LogRecord itself.
    // Thus improving the default bahavior for projects that log on other Logger instances not
    // created by BungeeCord.
  ...
  public void start() throws Exception {
    ...
    eventLoops =
        PipelineUtils.newEventLoopGroup(
            0, new ThreadFactoryBuilder().setNameFormat("Netty IO Thread #%1$d").build());
    ...
    if (config.isForgeSupport()) {
      ...
      getLogger()
          .warning(
              "MinecraftForge support is currently unmaintained and may have unresolved issues."
    ...
    saveThread.scheduleAtFixedRate(
        new TimerTask() {
          @Override
          public void run() {
    ...
    metricsThread.scheduleAtFixedRate(
        new Metrics(), 0, TimeUnit.MINUTES.toMillis(Metrics.PING_INTERVAL));
    ...
    for (final ListenerInfo info : config.getListeners()) {
      if (info.isProxyProtocol()) {
        getLogger()
            .log(
                Level.WARNING,
                "Using PROXY protocol for listener {0}, please ensure this listener is adequately"
                    + " firewalled.",
        ...
        if (connectionThrottle != null) {
          ...
          getLogger()
              .log(
                  Level.WARNING,
                  "Since PROXY protocol is in use, internal connection throttle has been"
                      + " disabled.");
        }
      }
            ...
            public void operationComplete(ChannelFuture future) throws Exception {
              if (future.isSuccess()) {
                ...
                getLogger()
                    .log(
                        Level.WARNING,
                        "Could not bind to host " + info.getSocketAddress(),
                        future.cause());
              }
            }
          };
      ...
      if (info.isQueryEnabled()) {
        Preconditions.checkArgument(
            info.getSocketAddress() instanceof InetSocketAddress,
            ...
            new ChannelFutureListener() {
              ...
              public void operationComplete(ChannelFuture future) throws Exception {
                if (future.isSuccess()) {
                  listeners.add(future.channel());
                  getLogger()
                      .log(Level.INFO, "Started query on {0}", future.channel().localAddress());
                } else {
                  getLogger()
                      .log(
                          Level.WARNING,
  ...
  private void independentThreadStop(final String reason, boolean callSystemExit) {
    ...
    // This needs to actually block here, otherwise running 'end' and then ctrl+c will cause the
    ...
    for (Plugin plugin : Lists.reverse(new ArrayList<>(pluginManager.getPlugins()))) {
      try {
        ...
        getLogger()
            .log(
                Level.SEVERE, "Exception disabling plugin " + plugin.getDescription().getName(), t);
      }
    ...
    // Unlock the thread before optionally calling system exit, which might invoke this function
    // again.
    // If that happens, the system will obtain the lock, and then see that isRunning == false and
  ...
  private void cacheResourceBundle(Map<String, Format> map, ResourceBundle resourceBundle) {
    ...
    while (keys.hasMoreElements()) {
      map.computeIfAbsent(
          keys.nextElement(), (key) -> new MessageFormat(resourceBundle.getString(key)));
    }
  }
  ...
  public PluginMessage registerChannels(int protocolVersion) {
    if (protocolVersion >= ProtocolConstants.MINECRAFT_1_13) {
      return new PluginMessage(
          "minecraft:register",
          String.join("\00", Iterables.transform(pluginChannels, PluginMessage.MODERNISE))
              .getBytes(StandardCharsets.UTF_8),
          false);
    }

    return new PluginMessage(
        "REGISTER", String.join("\00", pluginChannels).getBytes(StandardCharsets.UTF_8), false);
  }

  @Override
  public int getProtocolVersion() {
    return ProtocolConstants.SUPPORTED_VERSION_IDS.get(
        ProtocolConstants.SUPPORTED_VERSION_IDS.size() - 1);
  }

  @Override
  public String getGameVersion() {
    return ProtocolConstants.SUPPORTED_VERSIONS.get(0)
        + "-"
        + ProtocolConstants.SUPPORTED_VERSIONS.get(ProtocolConstants.SUPPORTED_VERSIONS.size() - 1);
  }
  ...
  public boolean addConnection(UserConnection con) {
    ...
    try {
      if (connections.containsKey(con.getName())
          || connectionsByUUID.containsKey(con.getUniqueId())
