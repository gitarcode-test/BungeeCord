...
/**
 * This class transforms chat components by attempting to replace transformable fields with the
 * appropriate value. <br>
 * ScoreComponents are transformed by replacing their {@link ScoreComponent#getName()}} into the
 * matching entity's name as well as replacing the {@link ScoreComponent#getValue()} with the
 * matching value in the {@link net.md_5.bungee.api.score.Scoreboard} if and only if the {@link
 * ScoreComponent#getValue()} is not present.
 */
...
public final class ChatComponentTransformer {
  ...
  /**
   * Transform a set of components, and attempt to transform the transformable fields. Entity
   * selectors <b>cannot</b> be evaluated. This will recursively search for all extra components
   * (see {@link BaseComponent#getExtra()}).
   *
   * @param player player
   * @param components the component to transform
   * @return the transformed component, or an array containing a single empty TextComponent if the
   ...
   *
   * @param player player
   * @param transformHover if the hover event should replace contents with value
  ...
  public BaseComponent transform(ProxiedPlayer player, boolean transformHover, BaseComponent root) {
    ...
    if (root.getExtra() != null && !root.getExtra().isEmpty()) {
      List<BaseComponent> list =
          root.getExtra().stream()
              .map((extra) -> transform(player, transformHover, extra))
  ...
  private void transformScoreComponent(ProxiedPlayer player, ScoreComponent component) {
    Preconditions.checkArgument(
        !isSelectorPattern(component.getName()), "Cannot transform entity selector patterns");
