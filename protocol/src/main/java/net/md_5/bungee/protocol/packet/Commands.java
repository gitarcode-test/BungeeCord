...
public class Commands extends DefinedPacket {
    ...
    for (int i = 0; i < nodeCount; i++) {
      ...
      switch (flags & FLAG_TYPE) {
        ...
        case NODE_ARGUMENT:
          ...
          argumentBuilder =
              RequiredArgumentBuilder.argument(name, ArgumentRegistry.read(buf, protocolVersion));

          if ((flags & FLAG_SUGGESTIONS) != 0) {
            String suggster = readString(buf);
            ((RequiredArgumentBuilder) argumentBuilder)
                .suggests(SuggestionRegistry.getProvider(suggster));
          }
          break;
        default:
  ...
  private static class ArgumentRegistry {
    ...
    private static final ArgumentSerializer<Boolean> BOOLEAN =
        new ArgumentSerializer<Boolean>() {
          @Override
    ...
    private static final ArgumentSerializer<FloatArgumentType> FLOAT_RANGE =
        new ArgumentSerializer<FloatArgumentType>() {
          @Override
    ...
    private static final ArgumentSerializer<DoubleArgumentType> DOUBLE_RANGE =
        new ArgumentSerializer<DoubleArgumentType>() {
          @Override
    ...
    private static final ArgumentSerializer<IntegerArgumentType> INTEGER_RANGE =
        new ArgumentSerializer<IntegerArgumentType>() {
          @Override
    ...
    private static final ArgumentSerializer<Integer> INTEGER =
        new ArgumentSerializer<Integer>() {
          @Override
    ...
    private static final ArgumentSerializer<LongArgumentType> LONG_RANGE =
        new ArgumentSerializer<LongArgumentType>() {
          @Override
    ...
    private static final ProperArgumentSerializer<StringArgumentType> STRING =
        new ProperArgumentSerializer<StringArgumentType>() {
          @Override
    ...
    private static ArgumentType<?> read(ByteBuf buf, int protocolVersion) {
      ...
      return val != null && PROPER_PROVIDERS.containsKey(val.getClass())
          ? (ArgumentType<?>) val
          : new DummyType(key, reader, val);
    }
  ...
  public static class SuggestionRegistry {
    ...
    private static String getKey(SuggestionProvider<DummyProvider> provider) {
      Preconditions.checkArgument(
          provider instanceof DummyProvider, "Non dummy provider " + provider);
    ...
    private static final class DummyProvider implements SuggestionProvider<DummyProvider> {
      ...
      public CompletableFuture<Suggestions> getSuggestions(
          CommandContext<DummyProvider> context, SuggestionsBuilder builder)
